...
The spring index is the ratio of the mean coil diameter of a spring to the wire diameter (D/d). 
This ratio is one of the most important considerations in spring design inasmuch as the deflection, stress, number of coils, and selection of either annealed or tempered material depends to a considerable extent upon this ratio. 
The best proportioned springs have an index of 7 to 9. 
Ratios of 4 to 7 and 9 to 16 are often used. 
Springs with values larger than 16 require more than standard tolerances for manufacturing; those with values less than
6 are difficult to coil on automatic coiling machines.

\subsection{Curvature Stress-Correction Factor (K)} % Section 21.11
(a) For helical compression and extension springs the curvature stress correction factor ($K$) is determined from the following formula:
$$ K = \frac{4C - 1}{4 C - 4} + \frac{0.615}{C} $$
The total stress, $S_\mathrm{max} = S_t \times K$

(b) For helical torsion springs the curvature stress-correction factor, ($K_1$) is determined from the following formula:
...
\subsection{Keystone Effect} % section 21.12
...
This formula may be used for both square and rectangular wire.

% page 66:
\subsection{Constants for Rectangular Wire} % section 21.13
The constants A and B in the formulas in Table II for compression and extension springs made from ~ aectiona
having either sharp or rounded edges are shown in Figure 86.


\subsection{Precautions and Suggestions for Effective Design of Compression Springs} % section 21.14
(yet to be transcribed)


\subsection{Tables of Spring Characteristics for Compression and Extension Springs} % section 21.15
\subsubsection{Example}
(yet to be transcribed)

\section{Design Nomographs for Compression and Extension Springs} % section 21.16
\subsubsection{Example}
(yet to be transcribed)

\section{Example of Compression Spring Calculation} % section 21.17
(yet to be transcribed)

\subsubsection{Necessity for Several Calculations} % section 21.17.1
...
% page 75:
\clearpage
\section{Stranded Wire Helical Compression Springs} % section 21.18
\subsection{General} % section 21.18.1
Helical compression springs made from standard wire have an inherent tendency about twice as high as round wire
springs to dampen high velocity displacement of their coils under shock loading. 
Under such conditions of loading, they have withstood 3 to 4 times as many deflections as round wire springs having the same load-deflection and stress conditions, before failure.
For this reason, they have been used for machine guns. 
They do not have longer life than round wire springs under normal types of load applications. 
Good results have been obtained by using three strands of music wire twisted so that the ratio of length of lay to the strand diameter is between 5 and 5.5.
The length of lay is the distance parallel to the strand axis in which a single wire makes one turn. 
Preforming the wire by twisting it slightly just prior to the actual stranding operation helps to keep the strands tightly together. 
Corrosion resisting steel wire also could be used, Springs with an index D/d of 13 can be coiled on automatic spring coilers, but springs with a smaller index usually require coiling over an arbor.
Shot peening is not recommended as the small shot lodges tightly between the strands and is difficult to detect and remove.

\subsection{Stress} % section 21.18.2
High stresses are used in design.
The following stresses have been used at solid length; for music wire 0.030 in. dia 170,000 to 195,000 and for music wire 0.070 in. dia 150,000 to 170,000 psi.

\subsection{Formulas for 3 Stranded Wire Spring Design} % section 21.18.3
The $S_t$ and $d$ values are for each strand of wire, and $P$ is the actual load on the spring.
$$ P = \frac{G d^4 F}{2.54 D N} $$
$$ S_t = \frac{G d F}{\pi N D^2} $$
$$ N = \frac{G d^4 F}{2.54 P D^2} $$
$$ r = \frac{P}{F} $$
The design procedure is the same as for other types of compression springs except that each strand carries its proportionate share of the load.
...
\subsection{General} % section 24.1
(yet to be transcribed)

\subsection{Stress} % section 24.2
(yet to be transcribed)

% table IX

\subsection{Ends} % section 24.3
(yet to be transcribed)

% page 101
\clearpage
\section{Constant Force Springs} % section 25
\subsection{General}% section 25.1
(yet to be transcribed)

% figure 55

\subsection{Materials} % section 25.2
(yet to be transcribed)

\subsection{Inner Ends} % section 25.3
Normally the inner end of a constant force extension spring is held to the roller by its natural gripping action, when about 1 1/2 turns of material remain on the roller at full deflectiom. 
No other fastening is mquired, except where them is some
tendency for the material to be wound onto the roller in improper alignment or if danger
of overtravel exists. 
In such cases the inner end may be bent and inserted into a slot in the roller, or retahed by a small upset hock
or held by a small screw. Inner ends held by
screws should be recesMMand amki iII a
manner to prevent the screw head fbm dforming
the mkxral curvature of the sp~o
Flangea onderaalso aidinguidingthe
flow of the material ontn the rollexa.

\subsection{Outer Ends} % section 25.4
(yet to be transcribed)
...
\subsection{Multiple Mounting} % section 25.5
(yet to be transcribed)

\subsection{Stress Factor}
(yet to be transcribed)

\subsection{Extension Type} % section 25.7
(yet to be transcribed)

\subsection{Formulas}
(yet to be transcribed)

\subsection{Simplified Design} % section 25.9
(yet to be transcribed)
...
\subsection{Example} % section 25.11
(yet to be transcribed)
...
% page 109
\clearpage
\section{Flat Springs} % section 26
\subsection{General} % section 26.1
Load requirements are intimakly
connectedwith spring dimensioning
and the space available for the spring. 
The
point of load application, deflectio~ length, width, and thickness should be clearly specified.
Formulas in the following table may be
used to determine various flat spring characteristics.

\subsection{Stress} % section 26.2
The stress is in bending and
should be compared with the elastic limit in
tension of the material to .deterrnine the allowable
stress. The recommended allowable
...
% page 109
\section{Coned Disc (Belleville) Springs} % section 27
\subsection{General} % section 27.1
(yet to be transcribed)

\subsection{Load Deflection} % section 27.2
(yet to be transcribed)

\subsection{Stress} % section 27.3
(yet to be transcribed)

\subsection{Loads} % section 27.4
(yet to be transcribed)

% page 111
\clearpage
\section{Recommended Maximum Working Stresses} % section 28
\subsection{Fatigue Strength Curves} % section 28.1
The fatigue strength curves Figure 62 are for the most popular spring materials. 
These are for compression springs, based on the minimum torsional elastic limit of each material. 
The values may be increased 25 per cent for springs that are properly stress relieved, cold set and shotpeened.

\subsubsection{Light Service} % section 28.1.1
This includes springs subjected to static loads or small deflections and seldom used springs such as those in bomb fuzes, projectiles, and safety devices.
This service is for 1000 to 10,000 deflections.

\subsubsection{Average Service} % section 28.1.2
This includes springs in general use in machine tools, mechanical products and electrical components.
Normal frequency of deflections not exceeding 3600 per hour permit such springs to withstand 100,000 to 1,000,000 deflections.

\subsubsection{Severe Service} % section 28.1.3
This includes springs subjected to rapid deflections over long periods of time and to shock loading such as in pneumatic hammers, hydraulic controls and valves.
This service is for 1,000,000 deflections and above.
Lowering the values 10 per cent permits 10,000,000 deflections. 

\subsubsection{Other Materials} % section 28.1.4
For materials not shown on the curves in Figure 62 the following multiplying constants may be used.

(a) For Beryllium Copper, multiply the values of the Phosphor Bronze curves by 1.20.

(b) For Spring Brass, multiply the values of the Phosphor Bronze curves by 0.75.

(c) For Mona multiply the values of the Inconel curves by .82.

(d) For K-Mon4 multiply the values of the Inconel cumws by .90.

(e) For Dumnickel use the same values as for Inconel.

(f) For head ~ (drawn to spring temper and precipitation hardened) multiply the values of the Inconel curves by 1.25.

(g) For Silico-Manganese, multiply the values of the Chmme-Van.adium curves by 0.90.

(h) For Chrome-SiLicon, multiply the values of the Chrome-Vanadium curves by 1.20.

(i) For Valve Spring Quality Wire, use the same values as for ChronM+Vanadhun.

(j) For Corrosion Resisting Steels type FS304 and FS420, multiply the vahm of the Corrosion Resisting Steel curves by .9S,

(k) For Corrosion Resisting Steel type 13316, multiply the values of the Corrosion Reais~ Steel curves by 0.90.

(l) For Corrosion Resisting Steels type AM 431and17-7PH,multiply the values of the Music Wire curves by 0.90.

\subsection{Permissible Elevated Temperatures} % section 28.2
Springs used at high temperatures exert less load and have larger deflections under load than at room temperature. Compression and extension springs subjected to the temperatures and stresses shown in the following table will have a loss of load of 5 per cent or less, (or if the load remains constant, they will deflect an additional 5 per cent), in 48 hours. Elastic limits and modulus values are also reduced, thus necessitating these lower allowable working stresses.

% Table XV. Permissible elevated temperatures for compression and extension springs. Loss of load at these temperatures is less than 5 percent in 48 hours.
...
(b) Cathodic electrocleaning, by making the work anode instead of cathode, reduces the amount of hydrogen evolved and shortens the time required in acid dipping.
This procedure is seldom used for springs because of the long time required to perform this operation for large quantities.

(c) Hydrogen evolution during electroplating can be reduced by using low current densities.
Low voltages (between 6 and 8 volts) may be used. 
Improper or speedup methods are harmful. 
Commercial electroplating of springs may be unsatisfactory, due to the high current densities commercially used which cause a greater evolution of hydrogen and therefore, more embrittlement.

(d) All electroplated springs made from steei Should be dehydrated as soon as possible after plating. 
A time delay of 5 hours may prove too long in some cases.

High carbon spring steel is more susceptible to hydrogen embrittlement than the low carbon steels, and faulty heat treatment prior to plating will produce a type of grain structure that tends to increase embrittement.
Springs should not be plated while in a stressed condition or when part of an assembly, beause stressed conditions increase embrittlement.
For this reason the stretching of extension springs, so that deposition will occur between the coils is not recommended.

Dehydrogenation, or removing of hydrogen, restores the normal condition of the steel. 
It is accomplished by heating the electroplated springs in an oven and should be done as soon as the springam plated and
washed. 
Correct temperatures and length of time at heat depend upon the type and size of the steel. 
Recommended heat for air circulating ovens is 375$^\circ$ $\pm$ 25$^\circ$ F for not less than 3 hours after plating.
Occasional embrittlement may occur even with the best regulated methods. 
Nonferrous materials are not ordinarily subject to hydrogen embrittlement during electroplating.
...
\begin{figure}[htbp] % h:here, t:top, b:bottom, p:page
    \centering
    \includegraphics[width=0.5\textwidth]{milstd29a_table_xvi.png}
    %\caption{}
    \label{tab:xvi}
\end{figure}
...
\begin{figure}[htbp] % h:here, t:top, b:bottom, p:page
    \centering
    \includegraphics[width=0.5\textwidth]{milstd29a_table_xvii.png}
    %\caption{}
    \label{tab:xvii}
\end{figure}
...
\begin{figure}[htbp] % h:here, t:top, b:bottom, p:page
    \centering
    \includegraphics[width=0.5\textwidth]{milstd29a_table_xviii.png}
    %\caption{}
    \label{tab:xviii}
\end{figure}
...
\begin{figure}[htbp] % h:here, t:top, b:bottom, p:page
    \centering
    \includegraphics[width=0.8\textwidth]{milstd29a_table_xix.png}
...
\begin{figure}[htbp] % h:here, t:top, b:bottom, p:page
    \centering
    \includegraphics[width=0.8\textwidth]{milstd29a_table_xxi.png}
    %\caption{Maximum Deviations Allowed on Ends of Extension Springs}
    \label{tab:xxi}
\end{figure}
...
\begin{figure}[htbp] % h:here, t:top, b:bottom, p:page
    \centering
    \includegraphics[width=0.8\textwidth]{milstd29a_table_xxii.png}
    %\caption{Maximum Deviations Allowed on Ends of Extension Springs}
    \label{tab:xxii}
\end{figure}
